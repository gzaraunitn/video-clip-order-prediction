...
def parse_args():
    ...
    parser.add_argument(
        "--pf", type=int, default=10, help="print frequency every batch"
    )
    parser.add_argument(
        "--seed", type=int, default=632, help="seed for initializing training."
    )
    ...
    if args.mode == "train":  ########### Train #############
        if args.ckpt:  # resume training
            ...
            if args.desp:
                ...
                exp_name = "{}_cl{}_it{}_tl{}_{}".format(
                    args.model, args.cl, args.it, args.tl, time.strftime("%m%d%H%M")
                )
        ...
        train_dataset = UCF101VCOPDataset(
            args.source_list, "data/ucf101", args.cl, args.it, args.tl, True, train_transforms
        )
        
        # split val for 800 videos
        train_dataset, val_dataset = random_split(
            train_dataset, (len(train_dataset) - 800, 800)
        )
        ...
        #             writer.add_video(
        #                 "train/tuple_clips", tuple_clips[:, i, :, :, :, :], i, fps=8
        #             )
        #             writer.add_text(
        #                 "train/tuple_orders", str(tuple_orders[:, i].tolist()), i
        #             )
        ...
        optimizer = optim.SGD(
            vcopn.parameters(), lr=args.lr, momentum=args.momentum, weight_decay=args.wd
        )
        scheduler = optim.lr_scheduler.ReduceLROnPlateau(
            optimizer, "min", min_lr=1e-5, patience=50, factor=0.1
        )
        ...
        for epoch in range(args.start_epoch, args.start_epoch + args.epochs):
            ...
            val_loss = validate(
                args, vcopn, criterion, device, val_dataloader, writer, epoch
            )
            ...
            if epoch % 20 == 0:
                torch.save(
                    vcopn.state_dict(),
                    os.path.join(log_dir, "model_{}.pt".format(epoch)),
                )
        ...
        test_dataset = UCF101VCOPDataset(
            "data/ucf101", args.cl, args.it, args.tl, False, test_transforms
        )
